...
import { getAppDataPath, ensureDir } from '../fs'
import { getLatestJREAsset, type AdoptiumAsset } from './adoptium'
    ...
    https.get(url, (response) => {
      if (response.statusCode === 301 || response.statusCode === 302) {
        ...
        if (!response.headers.location) {
          reject(new Error(`Redirected, but no location header provided (HTTP ${response.statusCode})`))
          return
        }
...
export async function setupJavaRuntime(
  ...
  const runtimeBaseDir = path.join(getAppDataPath(), 'runtime')
  ...
  onProgress?.(`Starting download: ${jreMeta.name} (${(jreMeta.size / 1024 / 1024).toFixed(1)} MB)`)
