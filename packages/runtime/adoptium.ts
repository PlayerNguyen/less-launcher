 ...
 */
export async function getLatestJREAsset(version: number | string): Promise<AdoptiumAsset> {
  ...
  if (!response.ok) {
    throw new Error(`Failed to fetch from Adoptium API (${response.status}): ${await response.text()}`)
  }
